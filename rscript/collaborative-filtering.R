...
# ************자바 연동 test************
#string test
# df <- data.frame(dbGetQuery(conn, query))
# df
# a <- df[1,1]
# a #이게되네???
...
rec_UBCF = Recommender(matrix_data, method = 'UBCF', param = list(method = 'pearson')) # 유사도 = pearson

#해당 id를 가진 사용자의 행번호 추출 (여기부터 자바에서 실행)
# who <- as.numeric(which(pivot_data$MEMBER_ID=='member2')) #who : member id의 row
# as(predict(rec_UBCF, matrix_data[who, ], type = 'topNList', n = 3), 'list') #결과는 list
