...
#*************test data************
raw_data <- analysis_data<-read.csv('C:/kopo_finalProject_API/최종플젝/통계/협업필터링/test.csv')
...
#히스토그램 그리기
#hist(raw_data$PIECE_NO, xlab="조각수", ylab="사용자수")
#hist(log(raw_data$PIECE_NO), xlab="조각수", ylab="사용자수")
...
for(i in 1:length(result)){
  result_vc[i]<- 2 * (result[[i]][20,6] * result[[i]][20,7]) / (result[[i]][20,6] + result[[i]][20,7])
}
